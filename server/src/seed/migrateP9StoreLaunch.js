...
/**
 * One-time migration: bring an already-seeded database's Store Launch (p9)
 * stage definition up to date with the collection-mode assessmentTypes now
 * defined in storeLaunchTemplate.js (Go-Live Approval/Final Store Inspection/
 * Inventory Verification/POS & Billing Activation/Staff Attendance
 * Verification/Marketing Launch/Store Opening Ceremony/Customer Go Live —
 * eight launch modules, replacing the old flat three-field schema).
 *
 ...
 * the p9 stage of every Template document that has one, in place, leaving
 * every project, record and user untouched.
 *
 * SAFE BY DEFAULT: runs as a DRY RUN and writes nothing. Pass --apply to persist.
 *
 *   node src/seed/migrateP9StoreLaunch.js            # preview only
 *   node src/seed/migrateP9StoreLaunch.js --apply     # actually migrate
 *
 * Touches only each template's `p9` stage (assessmentTypes, captureMode,
 * recordNoun, masterDataSchema) — no other stage, project, or record data is
 * modified. Any existing p9 Records still carry the old flat schema (no
...
async function migrate() {
  const templates = await Template.find({ 'stages.key': 'p9' });
  let touched = 0;

  for (const template of templates) {
    const stage = template.stages.find((s) => s.key === 'p9');
    if (!stage) continue;
  ...
  console.log(`\nTemplates with a p9 stage: ${touched}`);
  return touched;
}
